...
describe('@appendNode', () => {
  ...
  it('appends the mutation result to the connection of Root Query', async () => {
    const mocks = [
      { request: { query: queryDocument }, result: { data: queryMockData } },
      {
        request: { query: mutationDocument, variables: appendItemToRootMutationVariables },
        result: { data: mutationMockData },
      },
    ];
  ...
  it('appends the mutation result to connections with different args', async () => {
    const mocks = [
      {
